...
def main():   
            ...
            img = cv2.imread(r'20230901_fiber\label2\2_6_7_9_11_14\not_ave\910.png'.format(910), flags=cv2.IMREAD_GRAYSCALE)               
            mask = cv2.imread(r'20230901_fiber\mask\2102.png', flags=cv2.IMREAD_UNCHANGED)
            # n_labels, labels = cv2.connectedComponents(mask)

            # a1,a2 = np.where(labels!=500)
            # img[a1,a2] = 0
            ...
            # ax.plot_wireframe(X, Y, img, alpha=0.5)
            # ax.set_title('title')
