...
def CV_training (p):
    ...
    c = np.loadtxt(r'20230901_fiber\3Dcube\{}\abs.csv'.format(p[2]), delimiter=",")[:,:]
    ...
    d = np.loadtxt(r'20230901_fiber\3Dcube\{}\abs.csv'.format(p[3]), delimiter=",")[:,:]
    ...
    e = np.loadtxt(r'20230901_fiber\3Dcube\{}\abs.csv'.format(p[4]), delimiter=",")[:,:]
...
# =============================================================================
    ...
    model.add(keras.layers.Dropout(0.3))
    model.add(keras.layers.Dense(8,use_bias=True, activation='softmax'))
    #モデル評価方法の決定(要検討）
...
def main():
    ...
    print('テスト')
    tra0, tra1, tra2, tra3, tra4, tra5, tra6, tra7 =  CV_training(p) ####変える必要あり
    ...
    for i in range(0,fold_size):
        ...
        training_data = np.r_[t0_tra,t1_tra,t2_tra,t3_tra,t4_tra,t5_tra,t6_tra,t7_tra]####変える必要あり
        
        training_data_y = np.r_[t0_tra_y,t1_tra_y,t2_tra_y,t3_tra_y,t4_tra_y,t5_tra_y,t6_tra_y,t7_tra_y]####変える必要あり
        ...
        validation_data = np.r_[t0_val,t1_val,t2_val,t3_val,t4_val,t5_val,t6_val,t7_val]####変える必要あり
        
        validation_data_y = np.r_[t0_val_y,t1_val_y,t2_val_y,t3_val_y,t4_val_y,t5_val_y,t6_val_y,t7_val_y]####変える必要あり
        ...
        for m in range(len(metrics)):
            metric = metrics[m]
            plt.subplot(1, 3, m+1)  # figureを1×2のスペースに分け、i+1番目のスペースを使う
