...
#               970]
# rem_wave = [430,436,442,448,454,460,466,472,478,484,490,
#               970]
rem_wave = [430,436,442,448,454,460,466,472,478,484,490,496,502,508,514,520,526,532,538,544,550,556,562,568,574,580,586,592,598,604,
              610,616,622,628,634,640,646,652,658,664,670,676,682,688,694,700,706,712,718,724,730,736,742,748,754,760,766,772,778,784,
              790,796,802,808,814,820,826,832,838,844,850,856,862,868,874,880,886,892,898,904,910,916,922,928,934,940,946,952,958,964,
...
def CV_training (p):
    ...
    a = np.loadtxt(r'20230901_fiber\3Dcube\{}\{}\1000~1600\abs.csv'.format(num, p[0]), delimiter=",")[:,:]
    # a = remove_wavelength(a,rem_wave,wave_length)
    a = SG(a)
    np.random.shuffle(a) #教師データのランダム抽出
    aa = a[:10000,:] #抽出したい数を入力 #10000, 22615
    training_0=np.append(training_0, aa, axis=0)

    mag_p = int(mag * aa.shape[0]) #tumorの何倍いれるか

    b = np.loadtxt(r'20230901_fiber\3Dcube\{}\{}\1000~1600\abs.csv'.format(num, p[1]), delimiter=",")[:,:]
    ...
    c = np.loadtxt(r'20230901_fiber\3Dcube\{}\{}\1000~1600\abs.csv'.format(num, p[2]), delimiter=",")[:,:]
    ...
    d = np.loadtxt(r'20230901_fiber\3Dcube\{}\{}\1000~1600\abs.csv'.format(num, p[3]), delimiter=",")[:,:]
    ...
    e = np.loadtxt(r'20230901_fiber\3Dcube\{}\{}\1000~1600\abs.csv'.format(num, p[4]), delimiter=",")[:,:]
    ...
    f = np.loadtxt(r'20230901_fiber\3Dcube\{}\{}\1000~1600\abs.csv'.format(num, p[5]), delimiter=",")[:,:]
    ...
    g = np.loadtxt(r'20230901_fiber\3Dcube\{}\{}\1000~1600\abs.csv'.format(num, p[6]), delimiter=",")[:,:]
...
def gakusyuuki ():
    ...
    # model.add(keras.layers.Dropout(0.3))
    # model.add(keras.layers.Dense(2048, use_bias=True, activation='relu'))#5
    # model.add(keras.layers.Dropout(0.3))
    # model.add(keras.layers.Dense(1024, use_bias=True, activation='relu'))#6

    ####
    # model.add(keras.layers.Dropout(0.3))
    # model.add(keras.layers.Dense(2048, use_bias=True, activation='relu'))#4
    #####
...
# =============================================================================
    ...
    # model.add(keras.layers.Dense(512, use_bias=True, activation='relu'))
    # model.add(keras.layers.Dropout(0.3))
    # model.add(keras.layers.Dense(256, use_bias=True, activation='relu'))#3
    # model.add(keras.layers.Dropout(0.3))
    # model.add(keras.layers.Dense(128, use_bias=True, activation='relu'))#2
# =============================================================================
    # kokomade
    
    model.add(keras.layers.Dropout(0.3))
    model.add(keras.layers.Dense(7,use_bias=True, activation='softmax'))
    #モデル評価方法の決定(要検討）
...
def main():
    ...
    print('テスト')
    tra0, tra1, tra2, tra3, tra4, tra5, tra6 =  CV_training(p) ####変える必要あり
    #, tra7
    ...
    for i in range(0,fold_size):
        ...
        training_data = np.r_[t0_tra,t1_tra,t2_tra,t3_tra,t4_tra,t5_tra,t6_tra]####変える必要あり
        #,t7_tra
        training_data_y = np.r_[t0_tra_y,t1_tra_y,t2_tra_y,t3_tra_y,t4_tra_y,t5_tra_y,t6_tra_y]####変える必要あり
        #,t7_tra_y
        ...
        validation_data = np.r_[t0_val,t1_val,t2_val,t3_val,t4_val,t5_val,t6_val]####変える必要あり
        #,t7_val
        validation_data_y = np.r_[t0_val_y,t1_val_y,t2_val_y,t3_val_y,t4_val_y,t5_val_y,t6_val_y]####変える必要あり
        #,t7_val_y
        ...
        my_callbacks = [
            ...
            tf.keras.callbacks.ModelCheckpoint(filepath= r'20230901_fiber/NN/learning/{}otameshi.h5'.format(str(i)),save_best_only=True),
        ]
        ...
        for m in range(len(metrics)):
            ...
            pd.DataFrame(plt_train).to_csv(r'20230901_fiber/NN/learning/{}{}_gakushuritsu_train.csv'.format(str(i), metrics[m]))
            pd.DataFrame(plt_test).to_csv(r'20230901_fiber/NN/learning/{}{}_gakushuritsu_test.csv'.format(str(i), metrics[m]))
        plt.savefig(r'20230901_fiber/NN/learning/{}_gakushuritsu.png'.format(str(i)))
        plt.show()  # グラフの表示

        """"save the best model"""
        model.save(filepath=r'20230901_fiber/NN/learning/classification_model.h5', save_format='h5')
    ...
    os.rename(r'20230901_fiber/NN/learning/{}otameshi.h5'.format(str(best_model_number)), r'20230901_fiber/NN/learning/best_model.h5')
