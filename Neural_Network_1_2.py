...
# =============================================================================
    ...
    model.add(keras.layers.Dropout(0.3))
    model.add(keras.layers.Dense(2,use_bias=True, activation='softmax'))
    #モデル評価方法の決定(要検討）
...
def main():
    ...
    print('テスト')
    tra0, tra1 =  CV_training(p) ####変える必要あり
    # , tra5, tra6, tra7
    ...
    for i in range(0,fold_size):
        ...
        training_data = np.r_[t0_tra,t1_tra]####変える必要あり
        # ,t5_tra,t6_tra,t7_tra
        training_data_y = np.r_[t0_tra_y,t1_tra_y]####変える必要あり
        ...
        validation_data = np.r_[t0_val,t1_val]####変える必要あり
        # ,t5_val,t6_val,t7_val
        validation_data_y = np.r_[t0_val_y,t1_val_y]####変える必要あり
