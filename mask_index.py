...
def main():   
            ...
            #     print('=====')
            #     print(wl)
            mask = cv2.imread(r'20230901_fiber\mask\4\1000.png', flags=cv2.IMREAD_UNCHANGED)
            img = cv2.imread(r'20230901_fiber\mask\2100.png', flags=cv2.IMREAD_GRAYSCALE) 
            # 連結成分のラベリングを行う。
            n_labels1, labels1 = cv2.connectedComponents(mask)
            n_labels2, labels2 = cv2.connectedComponents(img)
            # # ラベル数
            ...
            # plt.show()
            # zero = img.copy()
            # zero = zero.astype(np.uint16)
            # zero[:,:] = 0
            # print(zero)
            index = []
            for i in range(n_labels1-1):
                i = i+1
                # print(i) 
                a1,a2 = np.where(labels1==i)
                # print(a1,a2)
                index.append(labels2[a1[0],a2[0]])
            print(index)
