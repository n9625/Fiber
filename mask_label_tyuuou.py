...
def main():   
            ...
            img = cv2.imread(r'20231113\ganma\2001.png', flags=cv2.IMREAD_GRAYSCALE)
            mask1 = img.copy()
            mask2 = img.copy()
            mask3 = img.copy()
            mask4 = img.copy()
            ...
            for i in range(retval-1):
                ...
                # cv2.circle(mask1, center=(int(x),int(y)), radius=3 , color=255, thickness=-1) 
                # cv2.circle(mask2, center=(int(x),int(y)), radius=3 , color=[0,0,255], thickness=-1) 

                cv2.circle(mask1, center=(int(x),int(y)), radius=2 , color=255, thickness=-1) 
                cv2.circle(mask2, center=(int(x),int(y)), radius=2 , color=[0,0,255], thickness=-1) 

                print(stats[i,4])
                r = math.sqrt(stats[i,4]/(math.pi)*1/2)

                cv2.circle(mask3, center=(int(x),int(y)), radius=int(r) , color=255, thickness=-1) 
                cv2.circle(mask4, center=(int(x),int(y)), radius=int(r) , color=[0,0,255], thickness=-1) 

            
            # cv2.imwrite(r'20230901_fiber\mask\{}.png'.format(2103), mask1)
            # cv2.imwrite(r'20230901_fiber\mask\{}.png'.format(2104), mask2)
            cv2.imwrite(r'20231113\mask\{}.png'.format(2105), mask3)
            cv2.imwrite(r'20231113\mask\{}.png'.format(2106), mask4)
            cv2.imwrite(r'20231113\mask\{}.png'.format(2109), mask1)
            cv2.imwrite(r'20231113\mask\{}.png'.format(2110), mask2)
