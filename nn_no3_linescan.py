...
def main():   
            ...
            img = cv2.imread(r'Hyperspectral_imaging_zyushi_03\henkango\NN\test\{}{}\NN_hm{}.png'.format(n,s,n), flags=cv2.IMREAD_UNCHANGED)
            mask = cv2.imread(r'Hyperspectral_imaging_zyushi_03\henkango\NN\test\{}{}\NN_hm{}.png'.format(n,s,n), flags=cv2.IMREAD_GRAYSCALE)
            # img = cv2.imread(r'Hyperspectral_imaging_zyushi_03\henkango\NN\test\{}\NN_hm{}.png'.format(n,n), flags=cv2.IMREAD_UNCHANGED)
            # mask = cv2.imread(r'Hyperspectral_imaging_zyushi_03\henkango\NN\test\{}\NN_hm{}.png'.format(n,n), flags=cv2.IMREAD_GRAYSCALE)
            data = cv2.imread(r'Hyperspectral_imaging_zyushi_03\henkango\whitecal\{}\910.png'.format(n), flags=cv2.IMREAD_GRAYSCALE)
            data = cv2.cvtColor(data, cv2.COLOR_GRAY2BGR)
            print(mask)
            print(img.shape)
            img1 = img.copy()
            # a1,a2 = np.where(mask == 29)

            # img1[a1,a2] = 0
            # mask[a1,a2] = 0
        
            b1,b2 = np.where(mask != 0)
            print(img1.shape, data.shape)
            data[b1,b2] = img1[b1,b2]

            cv2.imwrite(r'Hyperspectral_imaging_zyushi_03\henkango\NN\test\{}{}\NN_hm_{}-1.png'.format(n,s,n), img1)
            cv2.imwrite(r'Hyperspectral_imaging_zyushi_03\henkango\NN\test\{}{}\NN_hm_{}-2.png'.format(n,s,n), data)
            # cv2.imwrite(r'Hyperspectral_imaging_zyushi_03\henkango\NN\test\{}\NN_hm_{}-1.png'.format(n,n), img1)
            # cv2.imwrite(r'Hyperspectral_imaging_zyushi_03\henkango\NN\test\{}\NN_hm_{}-2.png'.format(n,n), data)
